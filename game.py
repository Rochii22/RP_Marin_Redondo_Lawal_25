...
class Game:
    ...
    def handle_input(self):
        """Handles keyboard input with jump system"""
        keys = pygame.key.get_pressed()
        
        # UP / W
        if (keys[pygame.K_UP] or keys[pygame.K_w]) and not self.key_pressed['up']:
        ...
        # LEFT / A
        if (keys[pygame.K_LEFT] or keys[pygame.K_a]) and not self.key_pressed['left']:
        ...
        # SPACE - quick jump upward
        if keys[pygame.K_SPACE] and not self.key_pressed.get('space', False):
            if self.player.start_jump('up'):
                self.sound_manager.play_sound('jump')
